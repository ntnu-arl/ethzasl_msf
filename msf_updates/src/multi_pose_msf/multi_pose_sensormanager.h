 ...
 */
#ifndef MULTI_POSE_SENSOR_MANAGER_H
#define MULTI_POSE_SENSOR_MANAGER_H
...
#include <msf_updates/MultiPoseSensorConfig.h>
...
typedef msf_updates::MultiPoseSensorConfig Config_T;
typedef dynamic_reconfigure::Server<Config_T> ReconfigureServer;
typedef shared_ptr<ReconfigureServer> ReconfigureServerPtr;

class MultiPoseSensorManager : public msf_core::MSF_SensorManagerROS<
    msf_updates::EKFState> {
  typedef MultiPoseSensorManager this_T;
 ...
 public:
  ...
  MultiPoseSensorManager(
      ros::NodeHandle pnh = ros::NodeHandle("~/multi_pose_sensor")) {
    imu_handler_.reset(
    ...
    //Customization
    init_scale_srv_ = pnh.advertiseService("initialize_msf_scale", &MultiPoseSensorManager::InitScale, this);
    init_height_srv_ = pnh.advertiseService("initialize_msf_height", &MultiPoseSensorManager::InitHeight, this);
    //Customization
  }
  virtual ~MultiPoseSensorManager() {
  }
 ...
 private:
  ...
  virtual void Config(Config_T &config, uint32_t level) {
    ...
    if ((level & msf_updates::MultiPoseSensor_INIT_FILTER)
    ...
    if ((level & msf_updates::MultiPoseSensor_SET_HEIGHT)
